...
{
    quint16 CRC_Result=0XFFFF;

    for(int CRC_count=0;CRC_count<unLength;CRC_count++)
    {
        int i;

        CRC_Result=CRC_Result^*(pszBuf+CRC_count);
...
unsigned short QuickCRC16(unsigned char * pMsg, unsigned short iSize , stAddr &stCRC16)
{
   ...
   {
      // Calculate the CRC
      index = stCRC16.cLow ^ ( unsigned char )( *pMsg++ );
      
      stCRC16.cLow = stCRC16.cHigh ^ HiCRCTable[index];
      stCRC16.cHigh = LoCRCTable[index];
   }
   return ( stCRC16.cHigh << 8 | stCRC16.cLow );
}
...
// POLYNOMIALS = 0X8005
// -----------------------------------------------------------------------------
unsigned short CRC16( unsigned char * pMsg, unsigned short iSize,stAddr& stCRC16)
{
   unsigned char  index;
   unsigned short iCRC;
